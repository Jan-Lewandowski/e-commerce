...
export default function Header() {
  ...
  const { isCartOpen, isFavoritesOpen, isAccountMenuOpen, toggleAccountMenu } =
    useApp();
    ...
    <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 shadow-sm backdrop-blur">
      <div className="mx-auto flex max-w-7xl flex-wrap items-center gap-3 px-4 py-3 sm:px-6 lg:flex-nowrap">
        <Link
          href="/"
          onClick={() => toggleTitle("")}
          className="flex min-w-fit items-center gap-3 rounded-lg focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-orange-500"
        >
          <Image
        ...
        <div className="ml-auto flex items-center justify-end gap-2 sm:gap-3">
          <div className="relative" ref={cartWrapRef}>
            <Button
              variant="ghost"
              onClick={toggleCart}
              className="relative h-11 w-11 rounded-xl border-slate-200 p-0 text-slate-700 hover:bg-orange-50 hover:text-orange-700"
            >
